...
static struct inode* new_inode(int,int,int,int);
...
static struct inode* create_file(char *path,int flags)
{
    ...
    struct inode *newino = new_inode(dir_inode->i_dev,inode_nr,free_sect_nr, flags);
...
static struct inode* new_inode(int dev,int inode_nr,int start_sect, int flags)
{
  	struct inode * new_inode = get_inode(dev, inode_nr);

	if(flags & FILE_FOLDER)
	...
	new_inode->i_father = root_inode->i_num;
	if(0 == new_inode->i_father)
		new_inode->i_father = 1;
	new_inode->i_filenum = 0;
