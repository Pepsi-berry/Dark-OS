...
void shabby_shell(const char *tty_name)
{
    ...
    while(1)
    {
        ...
        char random[16] = {};
        int randTemp = get_ticks();
        for(int i = 0; ; i++)
        {
            random[i] = randTemp%10;
            if(randTemp == 0)
                break;
            randTemp /= 10;
        }
...
void more_shabby_shell(const char *tty_name)
{
	int fd_stdin = open(tty_name, O_RDWR);
	assert(fd_stdin == 0);
	int fd_stdout = open(tty_name, O_RDWR);
	...
	while (1) 
	{
		printf("[root@RASPBERRYOS/%s]$ ",dir);

		rdbufLen = read(fd_stdin, rdbuf, 70);
		rdbuf[rdbufLen] = 0;
		...
		if(rdbufLen < MAXORDERLEN)
			Leff = rdbufLen;
		else
			Leff = MAXORDERLEN;
		
		getorder(rdbuf, order, MAXORDERLEN);
		
		respond(order, rdbuf, dir, fd_stdin, fd_stdout, rdbufLen, dirLen);
