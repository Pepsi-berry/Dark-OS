...
struct super_block {
	u32	magic;		  /**< 魔数 */
	u32	nr_inodes;	  /**< inode节点数量 */
	u32	nr_sects;	  /**< 扇区数量(including bit maps) */
...
struct inode {
	...
	u32	i_nr_sects;	/**< 文件占据了多少扇区How many sectors the file occupies */
	u32	i_father;	/*当前文件的父文件*/
	u32	i_filenum;	/*当前文件夹内存放的文件数*/
	u8	_unused[8];	/**< Stuff for alignment */

	/* the following items are only present in memory */
	int	i_dev;
	int	i_cnt;		/**<(共享此节点的进程数量)How many procs share this inode  */
	int	i_num;		/**< inode nr.  */
};
